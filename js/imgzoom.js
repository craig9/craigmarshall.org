...
// Copied from woodgears.ca and cleaned up the layout to make it more readable

// To use it, wrap images in an anchor tag, which has a href pointing
// to the full size image, and a bigw and a bigh denoting the full, zoomed-in 
// size.

// TODO speed up the zooming, perhaps also improve the variable names

function Loaded(id){
    oldimg = images[id];
    ni = par[id].nig;
    ni.align = oldimg.align;
    ni.width = oldimg.width;
    ni.height = oldimg.height;
    var os;
    if (oldimg.currentStyle) {
        os = oldimg.currentStyle;
    }
    else {
        os = document.defaultView.getComputedStyle(oldimg, null);
    }
    if (oldimg.offsetLeft > 540) {
        ni.align="right";
    }
    if (os.border != undefined) {
        ni.style.border = os.border;
    }
...
function MakeBig(id){
    ...
    if (!par[id].loaded){
        par[id].w_w = images[id].width;
        par[id].h_w = images[id].height;
    ...
    if (rivals.length){
        ...
        if (document.body && document.body.offsetWidth) {
            maxwsum=document.body.offsetWidth;
        }
        if (window.innerWidth && window.innerHeight) {
            maxwsum=window.innerWidth;
        }
        if (maxwsum > 1150) {
            maxwsum = 1150;
        }
...
function ScaleTimer(){
    prcdone = 0.02;
    // Reduced the divisor from 800 on the following line, to speed up the zoom transition
    prcmin = (new Date().getTime() - started) / 400; 
    ...
    sm = (1 - Math.cos(prcdone * 3.1416)) * 0.5;

    for (var id = 0; id < par.length; id++) {
        if (par[id].w_targ == par[id].w_is) {
            continue;
        }
        var img = images[id];
        var pm = par[id];
...
for(a=0; a<images.length; a++){
    ...
    par[a] = pa;
    if (par[a].w_b > 100) {
        imgp.setAttribute("href","javascript:MakeBig("+a+")");
    }
}
